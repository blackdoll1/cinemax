...
import { getSeatPrice, getSeatType, showtimes } from '../data/films';
...
export default function BookingPage() {
  ...
  // Séances du film sélectionné
  const filmShowtimes = showtimes.filter(s => s.movieId === selectedFilm.id);
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
          ...
          <div className="flex flex-wrap gap-2">
            {filmShowtimes.map(st => (
              ...
              >
                ...
                <span className={`text-[10px] mt-0.5 ${selectedShowtime?.id === st.id ? 'text-red-200' : 'text-[#888]'}`}>
      ...
      {/* ── Plan de salle (affiché seulement si une séance est sélectionnée) ── */}
      {selectedShowtime ? (
        <>
          {/* Écran */}
          ...
          <div className="bg-[#141414] border border-[#2E2E2E] rounded-xl px-5 py-4">
            {selectedSeats.length === 0 ? (
              <p className="text-[#555] text-sm">
                ...
                <span className="text-[#444]">(max 6)</span>
              </p>
            ) : (
                    ...
                    return (
                      ...
                      >
                        ...
                        <button
                          ...
                          className="ml-1 text-[#555] hover:text-red-400 transition-colors leading-none"
            ...
            {selectedSeats.length === 0 && (
              <button
                disabled
                className="mt-2 px-6 py-2.5 bg-[#1E1E1E] text-[#555] text-sm rounded-lg font-medium cursor-not-allowed float-right"
              >
                Confirmer →
              </button>
            )}
